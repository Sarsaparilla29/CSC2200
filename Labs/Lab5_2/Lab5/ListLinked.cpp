...
{
    // use temp to find previous ListNode of cursor
    ListNode* temp;
    for (temp = head; temp != cursor; temp = temp->next) {

    }
    // do the removing for ListNode pointed by cursor
    temp->next = cursor->next;
    // then remove cursor, that means update next pointer of cursor to head
    cursor->next = head;
    // update head to first ListNode of list
    head = cursor;
}
...
{
    // step 1 new ListNode(cursor->dataItem, cursor->next);
    // let cursor's next points to new ListNode
    // cursor->next =
    // step 2 update dataItem in cursor
    cursor->dataItem = newDataItem;
}
